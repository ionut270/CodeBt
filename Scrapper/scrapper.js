...
let scrapeItem = async data => {
	...
	if(bodyHTML.split(/<h1 class="card-title text-secondary text-center" style="font-size: 2.5em;">/)[1] === undefined ){
	...
	scraps.Title = JSON.stringify(bodyHTML.split(/<h1 class="card-title text-secondary text-center" style="font-size: 2.5em;">/)[1].split(/<\/h1>/)[0]);
	...
	if (content.structuredText.split(/<code class=" language-txt" style="white-space: pre-wrap;">/)[1] === undefined) {
		...
		txt += content.structuredText.split(/<code class=" language-[a-zA-Z1-9]*" style="white-space: pre-wrap;">/)[1].split(/<\/code>/)[0];
